...
import org.apache.samza.job.yarn.{ClientHelper, SamzaAppState}
...
class ApplicationMasterRestServlet(config: Config, state: YarnAppState, registry: ReadableMetricsRegistry) extends ScalatraServlet with ScalateSupport {
