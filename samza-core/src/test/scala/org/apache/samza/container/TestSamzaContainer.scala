...
class TestSamzaContainer extends AssertionsForJUnit with MockitoSugar {
  ...
  def testReadJobModel {
    ...
    val coordinator = new JobModelReader(jobModel, server)
    JobModelReader.jobModelRef.set(jobModelGenerator())
    coordinator.server.addServlet("/*", new JobServlet(JobModelReader.jobModelRef))
    try {
      coordinator.start
  ...
  def testReadJobModelWithTimeouts {
    ...
    val coordinator = new JobModelReader(jobModel, server)
    JobModelReader.jobModelRef.set(jobModelGenerator())
    val mockJobServlet = new MockJobServlet(2, JobModelReader.jobModelRef)
